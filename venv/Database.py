...
class MainWindow(QWidget):
    ...
    def _update_tt_day_table(self, week, day):
        self.cursor = self.conn.cursor()
        self.cursor.execute(
            f"SELECT id_time,id_subj,room_numb,id FROM timetable WHERE day={day + 1} AND is_even_week={self._week_to_type_bool(week)} ORDER BY id_time;")
    ...
    def _change_tt_day(self, row_num, week, day, id_code):
        ...
        if count > 0:
            ...
            try:
                self.cursor.execute(
                    f"UPDATE timetable SET id_time={int(row[0])}, id_subj={int(row[1])}, room_numb='{row[2]}', id={int(row[3])} WHERE id={int(id_code)};")
                self.conn.commit()
                self.cursor = self.conn.cursor()
            except:
    ...
    def _insert_tt_day(self, row_num, week, day):
        ...
        if count > 0:
            try:
                ...
                self.cursor.execute(
                    f"INSERT INTO timetable(id,is_even_week,day,id_time,id_subj,room_numb) VALUES ({int(row[3])},'{self._week_to_type_bool(week)}',{day + 1},{int(row[0])},{int(row[1])},'{row[2]}');")
                self.conn.commit()
                self.cursor = self.conn.cursor()
            except:
    ...
    def _change_times(self, row_num, id_code):
        ...
        if count > 0:
            ...
            try:
                self.cursor.execute(
                    f"UPDATE times SET id={int(row[0])}, start_time='{row[1]}', end_time='{row[2]}' WHERE id={int(id_code)};")
                self.conn.commit()
                self.cursor = self.conn.cursor()
            except:
                QMessageBox.about(self, "Error", "Update: Enter all fields")
    ...
    def _insert_times(self, row_num):
        ...
        if count > 0:
            try:
                self.cursor.execute(
                    f"INSERT INTO times(id,start_time,end_time) VALUES ({int(row[0])},'{row[1]}','{row[2]}');")
                self.conn.commit()
                self.cursor = self.conn.cursor()
            except:
                QMessageBox.about(self, "Error", "Add: Enter all fields")
    ...
    def _change_teachers(self, row_num, id_code):
        ...
        if count > 0:
            ...
            try:
                self.cursor.execute(
                    f"UPDATE teachers SET id={int(row[0])}, surname='{row[1]}', name='{row[2]}' , id_department={id_dep_value} WHERE id={int(id_code)};")
                self.conn.commit()
                self.cursor = self.conn.cursor()
            except:
                QMessageBox.about(self, "Error", "Update: Enter all fields")
    ...
    def _insert_teachers(self, row_num):
        ...
        if count > 0:
            try:
                self.cursor.execute(
                    f"INSERT INTO teachers(id,surname,name,id_department) VALUES ({int(row[0])},'{row[1]}','{row[2]}',{id_dep_value});")
                self.conn.commit()
                self.cursor = self.conn.cursor()
            except:
                QMessageBox.about(self, "Error", "Add: Enter all fields")
    ...
    def _change_dep(self, row_num, id_code):
        ...
        if count > 0:
            ...
            try:
                self.cursor.execute(
                    f"UPDATE departments SET id={int(row[0])}, link='{row[1]}', room_numb='{row[2]}' WHERE id={int(id_code)};")
                self.conn.commit()
                self.cursor = self.conn.cursor()
            except:
                QMessageBox.about(self, "Error", "Update: Enter all fields")
    ...
    def _insert_dep(self, row_num):
        ...
        if count > 0:
            try:
                self.cursor.execute(
                    f"INSERT INTO departments(id,link,room_numb) VALUES ({int(row[0])},'{row[1]}','{row[2]}');")
                self.conn.commit()
                self.cursor = self.conn.cursor()
            except:
                QMessageBox.about(self, "Error", "Add: Enter all fields")
    ...
    def _change_subj(self, row_num, id_code):
        ...
        if count > 0:
            ...
            try:
                self.cursor.execute(
                    f"UPDATE subject SET id={int(row[0])}, name='{row[1]}' , id_teacher={id_dep_value} WHERE id={int(id_code)};")
                self.conn.commit()
                self.cursor = self.conn.cursor()
            except:
                QMessageBox.about(self, "Error", "Update: Enter all fields")
    ...
    def _insert_subj(self, row_num):
        ...
        if count > 0:
            try:
                self.cursor.execute(
                    f"INSERT INTO subject(id,name,id_teacher) VALUES ({int(row[0])},'{row[1]}',{id_dep_value});")
                self.conn.commit()
                self.cursor = self.conn.cursor()
            except:
                QMessageBox.about(self, "Error", "Add: Enter all fields")
